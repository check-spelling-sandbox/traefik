...
func TestServerResponseEmptyBackend(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			...
			dialerManager := tcp.NewDialerManager(nil)
			dialerManager.Update(map[string]*dynamic.TCPServersTransport{"default@internal": {}})
			observabilityMgr := middleware.NewObservabilityMgr(staticConfig, nil, nil, nil, nil, nil)
			factory := NewRouterFactory(staticConfig, managerFactory, tlsManager, observabilityMgr, nil, dialerManager)
